...
public class SmartWebPlus extends BlackOutModule
{
    ...
    private void onTick(final TickEvent.Pre event) {
        if (this.timer.passedMs(this.delay.get()) && (!(boolean)this.onlyGround.get() || this.mc.player.isOnGround()) && this.pauseCheck() && InvUtils.find(Items.COBWEB).found()) {
            ...
            for (final PlayerEntity target : this.targets) {
                if (this.down.get()) {
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(0.0, -1.0, 0.0))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(0.1, -1.0, 0.1))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(-0.1, -1.0, 0.1))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(-0.1, -1.0, -0.1))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(0.1, -1.0, -0.1))));
                }
                if (this.face.get()) {
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(0.2, 1.0, 0.2))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(-0.2, 1.0, 0.2))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(-0.2, 1.0, -0.2))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(0.2, 1.0, -0.2))));
                }
                if (this.feet.get()) {
                    ...
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(0.2, 0.0, 0.2))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(-0.2, 0.0, 0.2))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(-0.2, 0.0, -0.2))));
                    this.placeWeb(new BlockPos(BOBlockUtil.vec3toBlockPos(target.getPos().add(0.2, 0.0, -0.2))));
                }
    ...
    private void placeWeb(final BlockPos pos) {
        if (BOBlockUtil.cantBlockPlace(pos)) {
            return;
        }
    ...
    private class Renderer
    {
        ...
        private void onRender(final Render3DEvent event) {
            if (!(boolean)SmartWebPlus.this.render.get()) {
                return;
            }
            SmartWebPlus.this.renderBlocks.removeIf(r -> System.currentTimeMillis() - r.time > 1000L);
            SmartWebPlus.this.renderBlocks.forEach(r -> {
                final double progress = 1.0 - Math.min(System.currentTimeMillis() - r.time + SmartWebPlus.this.renderTime.get() * 1000.0, SmartWebPlus.this.fadeTime.get() * 1000.0) / (SmartWebPlus.this.fadeTime.get() * 1000.0);
                event.renderer.box(r.blockPos, RenderUtils.injectAlpha(SmartWebPlus.this.sideColor.get(), (int)Math.round(SmartWebPlus.this.sideColor.get().a * progress)), RenderUtils.injectAlpha(SmartWebPlus.this.lineColor.get(), (int)Math.round(SmartWebPlus.this.lineColor.get().a * progress)), SmartWebPlus.this.shapeMode.get(), 0);
            });
        }
    }
}
