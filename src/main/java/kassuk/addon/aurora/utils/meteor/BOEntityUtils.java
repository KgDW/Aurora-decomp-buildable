package kassuk.addon.aurora.utils.meteor;
...
public class BOEntityUtils {
    ...
    public static boolean isBurrowed(PlayerEntity entity) {
        return BOEntityUtils.doesBoxTouchBlock(entity.getBoundingBox());
    }
    public static boolean doesBoxTouchBlock(Box box) {
        ...
        while ((double)x < Math.ceil(box.maxX)) {
            ...
            while ((double)y < Math.ceil(box.maxY)) {
                ...
                while ((double)z < Math.ceil(box.maxZ)) {
                    if (MeteorClient.mc.world.getBlockState(new BlockPos(x, y, z)).getBlock() == Blocks.OBSIDIAN || MeteorClient.mc.world.getBlockState(new BlockPos(x, y, z)).getBlock() == Blocks.BEDROCK || MeteorClient.mc.world.getBlockState(new BlockPos(x, y, z)).getBlock() == Blocks.ENDER_CHEST) {
                        return true;
                    }
                    ++z;
                }
