...
import kassuk.addon.aurora.managers.RotationManager;
import kassuk.addon.aurora.utils.NCPRaytracer;
import kassuk.addon.aurora.utils.OLEPOSSUtils;
import kassuk.addon.aurora.utils.RotationUtils;
        ...
        switch (this.mode(type)) {
            case Raytrace: {
                for (int r = 0; r < this.memory(type) && history.size() > r; ++r) {
                    ...
                    if (!this.raytraceCheck(rot.vec(), rot.yaw(), rot.pitch(), box)) continue;
                    return true;
                }
                break;
            }
            case StrictRaytrace: {
                for (int r = 0; r < this.memory(type) && history.size() > r; ++r) {
                    RotationManager.Rotation rot = history.get(r);
                    double range = 7.0;
                    Vec3d end = new Vec3d(range * Math.cos(Math.toRadians(rot.yaw() + 90.0)) * Math.abs(Math.cos(Math.toRadians(rot.pitch()))), range * -Math.sin(Math.toRadians(rot.pitch())), range * Math.sin(Math.toRadians(rot.yaw() + 90.0)) * Math.abs(Math.cos(Math.toRadians(rot.pitch())))).add(rot.vec());
                    if (!NCPRaytracer.raytrace(rot.vec(), end, box)) continue;
                    return true;
                }
                break;
            }
            case Angle: {
                for (int r = 0; r < this.memory(type) && history.size() > r; ++r) {
                    RotationManager.Rotation rot = history.get(r);
                    if (!this.angleCheck(rot.vec(), rot.yaw(), rot.pitch(), box, type)) continue;
                    return true;
                }
                break;
            }
